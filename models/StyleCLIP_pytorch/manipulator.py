...
class Manipulator():
    ...
    *** Attributes ***
    ...
    def __init__(
        ...
        self.W = torch.load(f'models/StyleCLIP_pytorch/tensor/W{dataset_name}.pt')
        self.S = torch.load(f'models/StyleCLIP_pytorch/tensor/S{dataset_name}.pt')
        self.S_mean = torch.load(f'models/StyleCLIP_pytorch/tensor/S_mean{dataset_name}.pt')
        self.S_std = torch.load(f'models/StyleCLIP_pytorch/tensor/S_std{dataset_name}.pt')
    ...
    def map_latent(self, latent_path):
        self.latent = torch.cat([torch.load(latent_path)])
        self.styles = {k: v.detach().clone() for k, v in self.G.mapping_stylespace(self.latent).items()}
