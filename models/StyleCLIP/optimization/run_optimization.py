...
STYLESPACE_INDICES_WITHOUT_TORGB = [i for i in range(len(STYLESPACE_DIMENSIONS))
                                    if i not in list(range(1, len(STYLESPACE_DIMENSIONS), 3))]

gender_samples_num = 6

def get_lr(t, initial_lr, rampdown=0.20, rampup=0.05):
...
def main(args):
    ...
    for i in pbar:
        ...
        if args.save_intermediate_image_every > 0 and i % args.save_intermediate_image_every == 0:
            ...
            torchvision.utils.save_image(img_gen, f"{args.results_dir}/{args.image_save_name}_{str(i).zfill(5)}.jpg", normalize=True,
                                         range=(-1, 1))

    if not (args.image_save_name is None):
        torchvision.utils.save_image(img_gen, f"{args.results_dir}/{args.image_save_name}_{loss.item():.4f}.png",
                                     normalize=True, range=(-1, 1))

    if not (args.latent_save_path is None):
        torch.save(latent, args.latent_save_path)
